...
export const loginUser = async (email, password) => {
  ...
  if (email === targetEmail && password === targetPassword) {
    ...
    if (!adminDbUser) {
      const error = new Error("Super Admin account not found in the database. Please seed the database first.");
      error.statusCode = 404;
      throw error;
    }

    const token = jwt.sign(
      { id: adminDbUser.id, role: "ADMIN", email },
    ...
    return {
      ...
      user: {
        ...
        email,
        role: "ADMIN",
        name: adminDbUser.employeeProfile?.legalName || "Super Admin",
...
export const changePassword = async (userId, currentPassword, newPassword) => {
  if (userId === "000000000000000000000000") {
    const error = new Error(
      "Super Admin password must be changed in environment variables.",
    );
    error.statusCode = 403;
    throw error;
  }
...
export const getMyProfile = async (userId, role) => {
  ...
  if (role === "EMPLOYEE" || role === "ADMIN") {
    ...
    return { type: role, profile: employee };
  }
