...
export const createRole = async (req, res) => {
    try {
        ...
        if (!validationResult.success) {
            return res.status(400).json({ status: 'fail', errors: validationResult.error.issues });
        }
        ...
        if (error.code === 'P2002') {
            return res.status(409).json({ status: 'fail', message: 'A role with this name already exists.' });
        }
...
export const updateRole = async (req, res) => {
    try {
        ...
        const role = await roleService.updateRole(req.params.id, validationResult.data.body);
